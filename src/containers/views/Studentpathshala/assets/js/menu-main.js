(function() {
  ...
  if( searchInput && navList) {
    ...
    mobileNavTrigger.addEventListener('click', function(event){
      ...
      var toggle = !MenuUtil.hasClass(sidebar, 'cd-side-nav--is-visible');
      if(toggle) expandSidebarItem();
      MenuUtil.toggleClass(sidebar, 'cd-side-nav--is-visible', toggle);
      MenuUtil.toggleClass(mobileNavTrigger, 'cd-nav-trigger--nav-is-visible', toggle);
    });
      ...
      (function(i){
        dropdownItems[i].children[0].addEventListener('click', function(event){
          if(checkMQ() == 'mobile') {
            ...
            MenuUtil.toggleClass(item, 'cd-side__item--expanded', !MenuUtil.hasClass(item, 'cd-side__item--expanded'));
          }
        });
      })(i);
    }
    ...
    new menuAim({
      ...
      activate: function(row) {
        MenuUtil.addClass(row, 'hover');
      },
      deactivate: function(row) {
        MenuUtil.removeClass(row, 'hover');
      },
      exitMenu: function() {
        hideHoveredItems();
        return true;
    ...
    function moveNavigation(){ // move searchInput and navList
      var mq = checkMQ();
      if ( mq == 'mobile' && !MenuUtil.hasClass(navList.parentNode, 'js-cd-side-nav') ) {
        detachElements();
        sidebar.appendChild(navList);
        sidebar.insertBefore(searchInput, sidebar.firstElementChild);
      } else if ( mq == 'desktop' && !MenuUtil.hasClass(navList.parentNode, 'js-cd-main-header') ) {
        detachElements();
    ...
    function hideHoveredItems() { // exit sidebar -> hide dropdown
      ...
      for(var i = 0; i < hoveredItems.length; i++) MenuUtil.removeClass(hoveredItems[i], 'hover');
    };
    ...
    function expandSidebarItem() { // show dropdown of the selected sidebar item
      MenuUtil.addClass(sidebar.getElementsByClassName('cd-side__item--selected')[0], 'cd-side__item--expanded');
    };

    function checkSelected(mq) {
      // on desktop, remove expanded class from items (js-cd-item--has-children) that were expanded on mobile version
      if( mq == 'desktop' ) {
        for(var i = 0; i < dropdownItems.length; i++) MenuUtil.removeClass(dropdownItems[i], 'cd-side__item--expanded');
      };
    }
  }
}());