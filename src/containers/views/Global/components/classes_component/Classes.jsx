...
const Classes = () => {
	...
	const addStream = () => {
		if (stream == '' && stream.length == 0) {
			setErrMsg('Please enter to Add Class.');
			setTimeout(() => {
				setErrMsg('');
			}, 1500);
			return false;
			...
			if (!stream.match(/^[a-zA-Z0-9\s(\'\",.+\-:)]+$/)) {
				setErrMsg('Please enter valid class.');
				setTimeout(() => {
					setErrMsg('');
				}, 1000);
				setStream('');
			...
			const streamInput = { class_name: stream, subjectids: subjectids };
			fetch('http://54.251.156.235:4001/classes', {
				method: 'POST',
				headers: {
					Accept: 'application/json',
	...
	const removeStream = (idd) => {
		fetch(`http://54.251.156.235:4001/classes/${idd}`, {
			method: 'delete',
			headers: {
				Accept: 'application/json',
				'Content-Type': 'application/json',
			...
			.then((response) => response.json())
			.then((responseData) => {
				setErrMsg('Class deleted successfully.');
				setTimeout(() => {
					setErrMsg('');
				}, 1500);
				allStream(gettoken);
	...
	const editStream = () => {
		if (updateStrm == '' && updateStrm.length == 0) {
			setErrMsg('Please enter to Update Class.');
			setTimeout(() => {
				setErrMsg('');
			}, 1500);
			return false;
			...
			const streamInput = { class_name: updateStrm };
			fetch(`http://54.251.156.235:4001/classes/${updateId}`, {
				method: 'PATCH',
				headers: {
					Accept: 'application/json',
					'Content-Type': 'application/json',
				...
				.then((response) => response.json())
				.then((responseData) => {
					setErrMsg('Class updated successfully.');
					setTimeout(() => {
						setErrMsg('');
					}, 1500);
										...
										<li key={v}>
											...
											<span>
												<a
													...
													onClick={() =>
														updateStream(txt.class_id, txt.class_name)
													}
													href="javascript:void(0);"
												>
													Edit
												...
												<a
													...
													href="javascript:void(0);"
													onClick={() => removeStream(txt.class_id)}
												>
													Delete
												</a>
											</span>
