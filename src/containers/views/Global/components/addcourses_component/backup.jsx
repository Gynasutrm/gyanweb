...
const Addcourses = () => {
	...
	const coursehandleChange = (event) => {
		setCoursedata({
			...coursedata,
			course_category_name:
				courselist[event.target.selectedIndex - 1].course_category_name,
			course_category_id:
				courselist[event.target.selectedIndex - 1].course_category_id,
		});
	};
	...
	const mediumhandleChange = (event) => {
		setMediumdata({
			...mediumdata,
			medium_name: mediumlist[event.target.selectedIndex - 1].medium_name,
			medium_id: mediumlist[event.target.selectedIndex - 1].medium_id,
		});
	};
	...
	const removeListbyIndex = (index) => {
		const finalArr = [...getDataAll];
		finalArr[0].subExamTypes.splice(index, 1);
		setGetDataAll(finalArr);
	};
							...
							<div className="grid_49 position_rel" style={{ float: 'right' }}>
								<div className="row_full position_rel">
									...
									<span
										onClick={() => {
											setHideshow1(!hideshow1);
											getDataAll.splice(0, getDataAll.length);
											setGetData2({});
											setGetData3([]);
										}}
										className="row_full multiSelect mar_b_10"
